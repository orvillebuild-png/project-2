  ...
  return (
    <>
      ...
      <Card className="max-w-5xl">
        <CardHeader
          description="Use merge fields: {{first_name}}, {{event_title}}, {{event_date}}, {{venue}}."
        ...
        <form action={createCampaign} className="grid gap-5 p-5">
          {error ? (
            <p className="rounded-md border border-[#f3c2b8] bg-[#fff0ed] px-3 py-2 text-sm text-coral">
              {error === "missing_fields" ? "Choose an event, subject, and message." : decodeURIComponent(error)}
            </p>
          ) : null}
